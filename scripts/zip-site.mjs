...
//
// Also copies it one folder up, next to the repo — a stray copy was found
// there (Aug 16, never touched by this script since), so whichever spot
// gets dragged into Netlify, both now stay current.
...
const parentZipPath = fileURLToPath(new URL('../../repertoire-lab-site.zip', import.meta.url));
...
copyFileSync(zipPath, parentZipPath);
console.log(`Also updated ${parentZipPath}`);
