...
function PlayerPage({ player, onBack, onAnalyze, onEditProfile }) {
  ...
  const index = useMemo(() => buildPositionIndex(state.openings), [state.openings]);
      ...
      {viewingGame && (
        <VariationViewer
          ...
          onAnalyze={(g) => {
            ...
            onAnalyze({ name: g.name, moves: g.moves, meta: g.meta, date: g.date });
          }}
        />
      )}
    </div>
...
// Landing: one section per person — you, and each student.
export default function GamesView({ onAnalyze }) {
  const { state, dispatch } = useStore();
  ...
  const index = useMemo(() => buildPositionIndex(state.openings), [state.openings]);
  const openPlayer = state.players.find((p) => p.id === openPlayerId);
  ...
  return (
    <div className="page">
      <div className="page-head">
        ...
        <button className="primary" onClick={() => setEditingPlayer('new')}>+ Add player</button>
      </div>
      <p style={{ color: 'var(--muted)', marginTop: -8 }}>
        A section per person — yourself and each student. Every game is matched against your
        repertoire (transpositions included), filed under the opening it belongs to, and one press
        away from the analysis board.
      </p>

      {state.players.length === 0 && (
        <div className="empty-note">
          No sections yet. Add one for yourself, then one per student.
        </div>
      )}
      ...
      {editingPlayer && (
        <PlayerEditor
          ...
          onClose={() => setEditingPlayer(null)}
          onSave={(name, profile) => {
            if (editingPlayer === 'new') dispatch({ type: 'addPlayer', name, profile });
